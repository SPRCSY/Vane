...
COUNTER=0
MAX_TRIES=30
until curl -s -H "X-Real-IP: 127.0.0.1" -H "X-Forwarded-For: 127.0.0.1" http://localhost:8080 > /dev/null 2>&1; do
  COUNTER=$((COUNTER+1))
...
if curl -s -H "X-Real-IP: 127.0.0.1" -H "X-Forwarded-For: 127.0.0.1" http://localhost:8080 > /dev/null 2>&1; then
...
exec node --max-old-space-size=16384 server.js
